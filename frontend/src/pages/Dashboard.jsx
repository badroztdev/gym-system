...
export default function DashboardPage() {
  ...
  const ACTIVITY_INFO = {
    member:       { icon: "👤", label: t("dashboard.activityNewMember"),       color: "var(--accent2)" },
    payment:      { icon: "💰", label: t("dashboard.activityPayment"),         color: "var(--accent)" },
    subscription: { icon: "🎫", label: t("dashboard.activityNewSubscription"), color: "var(--accent3)" },
  };
    ...
    <>
      ...
      <main style={{ padding: isMobile ? "14px 12px" : "24px 28px", flex: 1 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: isMobile ? 10 : 14, marginBottom: isMobile ? 10 : 14 }} className="stats-grid">
          <StatCard icon="👥" label={t("dashboard.statTotalAthletes")} value={o.members?.total_athletes || 0} sub={t("dashboard.statNewThisMonth", { count: o.members?.new_this_month || 0 })} color="var(--accent2)" delay={1} isMobile={isMobile} />
          <StatCard icon="🎫" label={t("dashboard.statActiveSubs")} value={o.subscriptions?.active_count || 0} sub={t("dashboard.statExpiringSoon", { count: o.subscriptions?.expiring_count || 0 })} color="var(--accent)" delay={2} isMobile={isMobile} />
          <StatCard icon="💰" label={t("dashboard.statRevenueMonth")} value={`${Number(o.revenue?.this_month || 0).toLocaleString()} ${t("common.currency")}`} sub={t("dashboard.statRevenueToday", { amount: `${Number(o.revenue?.today || 0).toLocaleString()} ${t("common.currency")}` })} color="var(--accent3)" delay={3} isMobile={isMobile} />
          <StatCard icon="⚠️" label={t("dashboard.statPendingDue")} value={`${Number(o.revenue?.total_due || 0).toLocaleString()} ${t("common.currency")}`} sub={t("dashboard.statOverdueCount", { count: o.subscriptions?.overdue_count || 0 })} color="var(--danger)" delay={4} isMobile={isMobile} />
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4, 1fr)", gap: isMobile ? 10 : 14, marginBottom: isMobile ? 18 : 24 }} className="stats-grid">
          <StatCard icon="📅" label={t("dashboard.statSessionsToday")} value={o.sessions?.today_count || 0} sub={t("dashboard.statSessionsWeek", { count: o.sessions?.week_count || 0 })} color="var(--accent2)" delay={1} isMobile={isMobile} />
          <StatCard icon="✅" label={t("dashboard.statAttendance30")} value={o.attendance?.present || 0} sub={t("dashboard.statAbsences", { count: o.attendance?.absent || 0 })} color="var(--accent)" delay={2} isMobile={isMobile} />
          <StatCard icon="⏰" label={t("dashboard.statLate30")} value={o.attendance?.late || 0} color="var(--warning)" delay={3} isMobile={isMobile} />
          <StatCard icon="🧑‍🏫" label={t("dashboard.statTeam")} value={(Number(o.staff?.coaches||0) + Number(o.staff?.assistants||0))} sub={t("dashboard.statTeamDetail", { coaches: o.staff?.coaches || 0, assistants: o.staff?.assistants || 0 })} color="var(--accent3)" delay={4} isMobile={isMobile} />
        </div>
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? 14 : 20, marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{t("dashboard.revenueChartTitle")}</div>
            <PeriodSelector value={revenuePeriod} onChange={setRevenuePeriod} periods={PERIODS} />
          </div>
          <ResponsiveContainer width="100%" height={240}>
            <LineChart data={revenue.map(r => ({ ...r, label: formatLabel(r.label, revenuePeriod), value: Number(r.value) }))}>
              ...
              <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} formatter={(v) => [`${v} ${t("common.currency")}`, t("dashboard.revenueTooltipLabel")]} />
        ...
        <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? 14 : 20, marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{t("dashboard.attendanceChartTitle")}</div>
            <PeriodSelector value={attendancePeriod} onChange={setAttendancePeriod} periods={PERIODS} />
          </div>
          <ResponsiveContainer width="100%" height={240}>
            <BarChart data={attendance.map(a => ({ ...a, label: formatLabel(a.label, attendancePeriod), present: Number(a.present), absent: Number(a.absent), late: Number(a.late) }))}>
              ...
              <Bar dataKey="present" name={t("dashboard.chartPresent")} fill="#6ee7b7" radius={[4,4,0,0]} />
              <Bar dataKey="late"    name={t("dashboard.chartLate")}    fill="#fbbf24" radius={[4,4,0,0]} />
              <Bar dataKey="absent"  name={t("dashboard.chartAbsent")}  fill="#f87171" radius={[4,4,0,0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.5fr 1fr", gap: 16, marginBottom: 16 }}>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? 14 : 20 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
              <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)" }}>{t("dashboard.growthChartTitle")}</div>
              <PeriodSelector value={growthPeriod} onChange={setGrowthPeriod} periods={PERIODS} />
            </div>
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={growth.map(g => ({ ...g, label: formatLabel(g.label, growthPeriod), new_members: Number(g.new_members) }))}>
                ...
                <Tooltip contentStyle={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: 8, fontSize: 12 }} />
                <Bar dataKey="new_members" name={t("dashboard.chartNewMembers")} fill="#818cf8" radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
          ...
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? 14 : 20 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 16 }}>{t("dashboard.ageDistTitle")}</div>
            {ageDist.length === 0 ? (
              <div style={{ textAlign: "center", padding: 40, color: "var(--muted)", fontSize: 12 }}>{t("dashboard.noData")}</div>
            ) : (
              <ResponsiveContainer width="100%" height={200}>
                <PieChart>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 16 }}>
          ...
          {/* أفضل المدربين */}
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? 14 : 20 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 14 }}>{t("dashboard.topCoachesTitle")}</div>
            {topCoaches.length === 0 ? (
              <div style={{ textAlign: "center", padding: 24, color: "var(--muted)", fontSize: 12 }}>{t("dashboard.noSufficientData")}</div>
            ) : (
                ...
                {topCoaches.map((c, i) => (
                  <div key={c.id} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    ...
                    <div style={{ textAlign: "left", flexShrink: 0 }}>
                      <span className="mono" style={{ fontSize: 12, color: "var(--accent2)" }}>{t("dashboard.sessionsCount", { count: c.sessions_count })}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {/* آخر الأنشطة */}
          <div style={{ background: "var(--card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", padding: isMobile ? 14 : 20 }}>
            <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text)", marginBottom: 14 }}>{t("dashboard.recentActivityTitle")}</div>
            {activity.length === 0 ? (
              <div style={{ textAlign: "center", padding: 24, color: "var(--muted)", fontSize: 12 }}>{t("dashboard.noRecentActivity")}</div>
            ) : (
                    ...
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 12, color: "var(--text)" }}>
                          ...
                          {a.type === "payment" && ` (${Number(a.detail).toLocaleString()} ${t("common.currency")})`}
